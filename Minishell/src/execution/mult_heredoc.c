...
/*   Updated: 2026/01/19 13:46:42 by oaletham         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static void	hd_child_loop(int write_fd, char *delimiter, t_cmd *cmds,
		t_exec_ctx x)
{
	char	*line;
	int		code;
	...
	while (1)
	{
		...
		tmp = expand_one(line, x.sh);
		free(line);
		line = tmp;
		if (is_delim(line, delimiter))
			return (free(line), hd_child_cleanup(write_fd, cmds, x, 0));
		write(write_fd, line, ft_strlen(line));
		write(write_fd, "\n", 1);
		free(line);
	}
	hd_child_cleanup(write_fd, cmds, x, code);
}
...
static pid_t	hd_fork_child(int fd[2], char *delimiter, t_cmd *cmds,
		t_exec_ctx x)
{
	pid_t	pid;
